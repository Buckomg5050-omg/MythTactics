...
public class UnitCombat : MonoBehaviour
{
    ...
    public IEnumerator PerformAbility(AbilitySO ability, Unit targetUnit, PlayerInputHandler attackerPIHContext)
    {
        ...
        {
            logMessageBuilder.Append(" Targeting ").Append(abilityTarget.unitName).Append(".");
        }
        ...
        if (needsHitRoll)
        {
            ...
            {
                abilityEffectConnects = CombatCalculator.ResolveAbilityHit(ability, _unitMain, abilityTarget);
            }
        }
        ...
        if (abilityEffectConnects)
        {
            ...
            {
                Unit effectRecipient = null;
                if (ability.targetType == AbilityTargetType.Self)
                {
                    effectRecipient = _unitMain;
                }
                else if (ability.targetType == AbilityTargetType.AllyUnit || ability.targetType == AbilityTargetType.EnemyUnit)
                {
                    effectRecipient = abilityTarget;
                }
                ...
                else if (effectRecipient != null && !effectRecipient.IsAlive)
                {
                     DebugHelper.Log($"Effect recipient {effectRecipient.unitName} for ability '{ability.abilityName}' is not alive. Effects not applied.", _unitMain);
                }
            }
